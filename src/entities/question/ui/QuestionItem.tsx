...
const QuestionItem = ({ question }: QuestionItemProps) => {
  ...
  const { answers, loading, error, deleteAnswer, refetch, editAnswer, markAsCorrect } = useAnswers(question.id, question.isResolved);
  ...
  return (
    <div className="question-item">
      ...
      {question.answers.length > 0 && <AnswerseAccordion markAsCorrect={markAsCorrect} isOwner={isOwner} editAnswer={editAnswer} deleteAnswer={deleteAnswer} answers={answers} />}
    </div>
  );
};
